...
class DeopCommand extends BaseCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        if ($player === null) {
            $sender->sendMessage('§r§cYou have to specify a player to take operator from!');
            return;
        }
        ...
        PlayerSqlHelper::setOpByName($name, false, function (bool $changed) use ($name, $sender): void
        {
            if (!$changed) {
                $sender->sendMessage('§r§cThat player is not an operator!');
                return;
            }

            $sender->sendMessage('§r§7' . $name . ' is no longer an operator!');
