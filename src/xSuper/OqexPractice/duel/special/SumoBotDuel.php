...
use xSuper\OqexPractice\entities\bots\sumo\DummySumoBot;
use xSuper\OqexPractice\entities\bots\sumo\EasySumoBot;
use xSuper\OqexPractice\entities\bots\sumo\GodlySumoBot;
use xSuper\OqexPractice\entities\bots\sumo\HardSumoBot;
use xSuper\OqexPractice\entities\bots\sumo\NormalSumoBot;
...
class SumoBotDuel extends Duel
{
    protected ?PracticePlayer $winner = null;
    private ?SumoEntity $bot = null;
    ...
    public function init(PluginBase $plugin): void
    {
        ...
        MapGenerator::genMap($plugin, $plugin->getServer()->getDataPath(), $this->getWorldName(), $plugin->getServer()->getDataPath() . '/worlds/' . $this->getWorldName(), function() use ($plugin, $map): void{
            ...
            if($this->player->isOnline()){
                ...
                $this->player->setCanBeDamaged(false);
    ...
    public function start(PluginBase $plugin): void
    {
        ...
        if ($this->player->isOnline()) {
            ...
            $e = match ($this->botType) {
                BotType::DummySumo => new DummySumoBot(Location::fromObject($pos, $world), $this->player, null),
                BotType::EasySumo => new EasySumoBot(Location::fromObject($pos, $world), $this->player, null),
                BotType::NormalSumo => new NormalSumoBot(Location::fromObject($pos, $world), $this->player, null),
                BotType::HardSumo=> new HardSumoBot(Location::fromObject($pos, $world), $this->player, null),
                BotType::GodlySumo => new GodlySumoBot(Location::fromObject($pos, $world), $this->player, null),
            };
        ...
        $tick = 0;
        $plugin->getScheduler()->scheduleRepeatingTask(new ClosureTask(function () use ($plugin, $min, &$tick): void {
            ...
            if ($this->bot !== null) $dist2 = $this->bot->getPosition()->getY();
            else $dist2 = null;

            if ($this->player->getPosition()->getY() <= $min && $this->winner === null) {
                $e = true;
                if ($this->bot !== null) $this->bot->tBag = true;
            } else if ($dist2 !== null && $dist2 <= $min && $this->winner === null) {
                $e = true;
                $this->winner = $this->player;
            }

            if ($e) $this->end($plugin);

            $tick++;
            if ($tick % 20 === 0) $this->time++;
        }), 1);
    }
