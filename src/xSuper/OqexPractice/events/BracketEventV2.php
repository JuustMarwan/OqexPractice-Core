...
class BracketEventV2 extends Event
{
    private const KITS = [
        'NoDebuff',
        'Debuff',
    ...
    {
        $players = [];
        foreach ($this->getRealPlayers() as $p) $players[] = $p->getName();

        if ($this->ladder === null) $this->ladder = new EventLadder($players);
        ...
        if (count($players) > $this->minForLarge()) {
            $this->largerLadder = new EventLadder($players);
    ...
    public function createFight(): void
    {
        ...
        if (is_array($r)) {
            ...
            OqexPractice::getInstance()->getScheduler()->scheduleDelayedTask(new ClosureTask(function () use ($r, $p1UuidString, $p2UuidString): void {
                $p1 = Server::getInstance()->getPlayerExact($r[0]);
                $p2 = Server::getInstance()->getPlayerExact($r[1]);
    ...
    public function disqualify(PracticePlayer|string $player): void
    {
        ...
        if ($this->largerLadder !== null) {
            if ($this->waiting <= 0) {
                if (count($this->ladder->getPlayers()) > $this->minForLarge()) {
