...
class DuelRequestForm extends MenuForm
{
	private null|PracticePlayer|string $p;

	public function create(Player $player, null|PracticePlayer|string $p): BaseForm
	{
		$this->p = $p;
    ...
    {
        if ($this->p instanceof PracticePlayer) return 'Send a duel request';
        else return 'Select a duel type';
    }
    ...
    {
        if ($this->p instanceof PracticePlayer) {
            return [
    ...
    {
        if ($this->p instanceof PracticePlayer) $type = match ($selected) {
        ...
        else $type = match ($selected) {
            0 => Type::getType('NoDebuff'),
            1 => Type::getType('Debuff'),
            2 => Type::getType('Gapple'),
            3 => Type::getType('BuildUHC'),
            4 => Type::getType('Combo'),
            5 => Type::getType('Vanilla'),
            6 => Type::getType('Archer'),
            7 => Type::getType('Soup'),
            default => null
        };

        if ($type !== null && $player instanceof PracticePlayer) {
            Forms::MAP_SELECTION()->create($player, $this->p, $type);
        }
    }
}